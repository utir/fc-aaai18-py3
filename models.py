...
def plot_source(cs, probs, vera, save_name = 'abc.pdf'):
    ...
    p1 = plt.barh(list(range(m)), probs[:, 0], color = colors[0], hatch = '//')
    p2 = plt.barh(list(range(m)), probs[:, 1], left = probs[:, 0], color = colors[1])
    p3 = plt.barh(list(range(m)), probs[:, 2], left = probs[:, 0] \
    ...
    p1 = plt.barh(list(range(m)), vera[:, 0], color = colors[0], hatch = '//')
    p2 = plt.barh(list(range(m)), vera[:, 1], left = vera[:, 0], color = colors[1])
    p3 = plt.barh(list(range(m)), vera[:, 2], left = vera[:, 0] \
...
def plot_probs(probs, reps, ss, vera, save_name = 'abc.pdf'):
    ...
    plt.bar(list(range(3)), vera, color = colors[3])
    plt.xticks(list(range(3)), ['False', 'Unk.', 'True'])
    
    plt.subplot(gs[0]) 
    plt.xlabel('Source Reputation')
    plt.yticks( np.arange(m) + 0.0, ss)
    
    p1 = plt.barh(list(range(m)), reps * probs[:, 0], color = colors[0], hatch = '//')
    p2 = plt.barh(list(range(m)), reps * probs[:, 1], left = reps * probs[:, 0], color = colors[1])
    p3 = plt.barh(list(range(m)), reps * probs[:, 2], left = reps * probs[:, 0] \
