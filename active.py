...
class sim:
    ...
    def query(self, indices):
        ...
        f1 = [x in aids for x in self.cds_test.data.aid]
...
class active_selector:
    ...
    def run(self):
        ...
        self.sl_ids = self.rs.choice(list(range(self.n_test)), size = self.k, replace = False, \
...
class experiment:
    ...
    def run(self, train_data, X_train, test_data, X_test, cds, cdv, cds_test,\
        ...
        runs = self.runs
        if seeds == None: seeds = list(range(runs))
        for seed in seeds:
...
def plot_curves(saves, conds = ['Baseline', 'Ours'], time = None, xlab = '', ylab = '',\
    ...
    markers = ['s', 'o', '^', 'v', '<']
    if time == None: time = list(range(len(saves[0][0])))
