...
def plot_claims(dic_claims):
    for c, docs in list(dic_claims.items()):
        if True:
...
def baseline(train_data, X_train, test_data, X_test, return_proba = False, \
    ...
    # classify stances
    if clf_stance == None:
        clf_stance = sklearn.linear_model.LogisticRegression(penalty = 'l1', solver="liblinear")
    ...
    n = len(train_claims)
    m = list(dic_f.items())[0][1].shape[0]
    ...
    clf_claim = sklearn.linear_model.LogisticRegression(solver='liblinear')
    ...
    n = len(test_claims)
    m = list(dic_f_test.items())[0][1].shape[0]
...
def stance_num2lab(a):
    ...
    return [dic_s[x] for x in a]

def vera_num2lab(a):
    dic_v = {0: 'false', 1: 'unknown', 2: 'true'}        
    return [dic_v[x] for x in a]
...
def baseline_crowd(train_data, X_train, test_data, X_test, cds, cdv, \
    ...
    #dss.em(5)
    dss.em(ds_it)
    dss.get_full_pos(list(range(len(train_data) + len(test_data) + 1)))
    ...
    for i in range(len(train)):
        ...
        train.at[j, 'articleHeadlineStance'] = dss.dic_al[aid]
    ...
    for i in range(len(test)):
        ...
        if aid in dss.dic_al:
            ...
            test.at[j, 'articleHeadlineStance'] = dss.dic_al[aid]
        else:
            # test.set_value(j, 'articleHeadlineStance', "none")
            test.at[j, 'articleHeadlineStance'] = "none"
...
def build_claim_df(data):
    ...
    n_claims = len(dic_hl)
    df = pd.DataFrame(index = list(range(n_claims)), columns = ['claimCount', 'headlines', \
                      'claimH'] )
    
    hl = list(dic_hl.items())
...
def build_claim_df2(data, dc):
    ...
    n_claims = len(dic_cc)
    df = pd.DataFrame(index = list(range(n_claims)), columns = ['claimCount', 'stances', \
                      'claimH'] )
    
    hl = list(dic_cc.items())
    for i in range(n_claims):
        df.loc[i] = list(hl[i]) + [ dic_ch[hl[i][0]] ]
    return df
...
def baseline_with_body(train_data, X_train, test_data, X_test, return_proba = False, \
    ...
    n = len(train_claims)
    m = list(dic_f.items())[0][1].shape[0]
    m2 = list(dic_f2.items())[0][1].shape[0]
    ...
    n = len(test_claims)
    m = list(dic_f_test.items())[0][1].shape[0]
    m2 = list(dic_f2_test.items())[0][1].shape[0]
...
def snopes_read(loc=''):
    ...
    a.ArticleHeadline = list(map(strip, a.ArticleHeadline))
    a['source'] = list(map(get_source, a.URL))
    ...
    a['claimTruth']  = list(map(get_vera, a.Claim))
    ...
    a.articleHeadline = [x.decode('utf-8', errors='ignore') for x in a.articleHeadline]
    a.claimHeadline = [x.decode('utf-8', errors='ignore') for x in a.claimHeadline]
...
def snopes_split(a):
    ...
    a_train.index = list(range(len(a_train.index)))
    a_val.index   = list(range(len(a_val.index)))
    a_test.index  = list(range(len(a_test.index)))
