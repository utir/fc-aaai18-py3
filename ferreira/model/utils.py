...
def get_contraction_mappings():
    ...
    with open(os.path.join(folder, 'contraction_map.csv')) as f:
        return dict([_f for _f in csv.reader(f) if _f])
...
@lru_cache(maxsize=1000)
def get_synonyms(w):
    return set([x.lower() for x in flatten([ss.lemma_names() for ss in wn.synsets(w.lower())])])
...
def get_brown_cluster_data(num_classes):
    brown_clusters = list(_brown_cluster_data_files_by_size.keys())
...
def compute_paraphrase_score(s, t):
    ...
    matches = [a_b_c for a_b_c in s_paraphrases if a_b_c[0] == t or a_b_c[0] == t_stem]
    if matches:
        return max(matches, key=lambda x_y_z: x_y_z[1])[1]
...
def get_svo(grph, grph_labels, node=0, svo=None):
    if svo is None:
        svo = []
    svo_match = dict([(l, (x, y)) for ((x, y), l) in list(grph_labels.items()) \
...
def get_svo_triples(id):
    ...
    d = []
    for i, x in list(stanparse_depths[id].items()):
