...
with open("edata_pp_020.pkl", 'rb') as f:
    (train_data_pp, X_train, val_data_pp, X_val, test_data_pp, X_test) = pickle.load(f, encoding="latin1")
...
data17.ans = list(map(lambda x: stance_dic[x], data17.ans))

# 2400 - end with rationale
data18 = crowd.read_batch_data(data_tvt, fn = 'mturk/Batch_2794184_batch_results.csv')
data18.ans = list(map(lambda x: stance_dic[x], data18.ans))
...
def offline_experiment():
    classifiers_to_pickle = util.export(train_val_data_pp, X_train_val)
    pickle.dump(classifiers_to_pickle, open('pickle-test.pkl', 'wb'))

    # If you want to bypass the paper verification and just export the classifier
    # comment out everything below inside offline_experiment()
    ...
    for seed in range(10, 11, 1):
        ...
        for i in range(6):
            ...
            # crowd model with inference by variational method, inherits from crowd_model
