...
class CD:
    ...
    def __init__(self, data, lab2num = None, labtype = 'stance'):
        ...
        self.mvc = list(map(lambda x: self.lab2num[x], self.mvl))
    ...
    def to_code(self):
        ...
        """
        ac = list(map(lambda x: self.a.index(x), self.data.aid))
        self.data = self.data.assign(ac = ac)
        wc = list(map(lambda x: self.w.index(x), self.data.wid))
        self.data = self.data.assign(wc = wc)
        # pd.set_option('display.max_rows', None)
        lc = list(map(lambda x: self.lab2num[str(x).lower()], self.data.ans))
...
class DS:
    ...
    def em(self, n_its = 5):
        ...
        self.mll = list(map(lambda x: self.cd.num2lab[x], self.mlc))
